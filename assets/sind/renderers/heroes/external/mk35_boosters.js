function create(renderer, icon, backBoosters) {
    ...
    handR.setOffset(1.125, 18.0, 0.0).setSize(2.0, 2.0);
    handR.anchor.set("rightArm");

    var handL = renderer.createEffect("fiskheroes:booster").setIcon(icon);
    handL.setOffset(-1.125, 18.0, 0.0).setSize(2.0, 2.0);
    ...
    if (backBoosters) {
        ...
        back.setOffset(1.5, 2.6, 1.75).setSize(1.25, 3.0);
        back.anchor.set("body");
        back.mirror = true;

        back2 = renderer.createEffect("fiskheroes:booster").setIcon(icon);
        back2.setOffset(0.0, 3.6, 1.75).setSize(1.5, 3.0);
        back2.anchor.set("body");
        back2.mirror = false;
    }
            ...
            if (!isFirstPersonArm) {
                if (all || renderLayer == "CHESTPLATE") {
                    ...
                    if (back != null && entity.getData('fiskheroes:suit_open_timer') == 0) {
                        ...
                        back2.progress = entity.getInterpolatedData("fiskheroes:dyn/booster_timer");
                        back2.speedScale = 0.5 * boost;
                        back2.setRotation(25 - 10 * boost, 0.0, 0);
                        back2.render();
                    }
                }
